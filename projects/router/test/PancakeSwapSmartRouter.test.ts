...
const PancakeStableSwapThreePoolDeployer = artifacts.require(
  'stable-swap/contracts/PancakeStableSwapThreePoolDeployer.sol'
)
...
const PancakeStableSwapThreePoolInfo = artifacts.require(
  'stable-swap/contracts/utils/PancakeStableSwapThreePoolInfo.sol'
)
...
contract('SmartRouter', ([admin, bob, carol]) => {
  ...
  before(async () => {
    ...
    stableSwapFactory = await PancakeStableSwapFactory.new(
      stableSwapLPFactory.address,
      stableSwap2PoolDeployer.address,
      stableSwap3PoolDeployer.address,
      {
        from: admin,
      }
    )
    ...
    stableSwapPoolInfo = await PancakeStableSwapInfo.new(stableSwap2PoolInfo.address, stableSwap3PoolInfo.address, {
      from: admin,
    })
    ...
    await stableSwapFactory.createThreePoolPair(BUSD.address, USDC.address, USDT.address, A, Fee, AdminFee, {
      from: admin,
    })
    ...
    await stable2Pool_BUSD_USDC.add_liquidity([parseEther('1000000'), parseEther('1000000')], parseEther('2000000'), {
      from: bob,
    })
    ...
    await stable2Pool_WBNB_BUSD.add_liquidity([parseEther('100'), parseEther('20000')], parseEther('0'), {
      from: bob,
    })
  ...
  describe('Check setups and info', () => {
    it('Check Stable Swap', async () => {
      ...
      await expectRevert(
        stable3Pool_BUSD_USDC_USDT.add_liquidity(
  ...
  describe('User swap via SmartRouter', () => {
    it('Carol exact output swap BUSD -> USDC on stable 2pool & exact output swap USDC -> 100 USDT on stable 3pool', async () => {
      ...
      const exactOnputData = smartRouter.contract.methods
        .exactOutputStableSwap([BUSD.address, USDC.address, USDT.address], [2, 3], parseEther('100'), parseEther('105'))
        .encodeABI()
    ...
    it('Carol exact input swap 100 BUSD -> USDC on stable 2pool & exact output swap USDC -> 100 USDT on stable 3pool', async () => {
      ...
      const exactInput2PoolData = smartRouter.contract.methods
        .exactInputStableSwap([BUSD.address, USDC.address], [2], parseEther('100'), 0)
        .encodeABI()
      const exactOnput3PoolData = smartRouter.contract.methods
        .exactOutputStableSwap([USDC.address, USDT.address], [3], parseEther('100'), parseEther('105'))
        .encodeABI()
    ...
    it('Carol exact output swap BUSD -> 100 USDC on stable 2pool & exact output swap BUSD -> 100 USDC on stable 3pool', async () => {
      ...
      const exactOutput2PoolData = smartRouter.contract.methods
        .exactOutputStableSwap([BUSD.address, USDC.address], [2], parseEther('100'), parseEther('105'))
        .encodeABI()
      const exactOnput3PoolData = smartRouter.contract.methods
        .exactOutputStableSwap([BUSD.address, USDC.address], [3], parseEther('100'), parseEther('105'))
        .encodeABI()
    ...
    it('Carol exact input swap 100 BUSD -> USDC on stable 2pool & exact output swap BUSD -> USDT -> 100 USDC on stable 3pool', async () => {
      ...
      const exactInput2PoolData = smartRouter.contract.methods
        ...
        .encodeABI()
      const exactOnput3PoolData = smartRouter.contract.methods
        .exactOutputStableSwap([BUSD.address, USDT.address, USDC.address], [3, 3], parseEther('100'), parseEther('105'))
        .encodeABI()
      const result = await smartRouter.multicall(deadline, [exactInput2PoolData, exactOnput3PoolData], { from: carol })
    })
    ...
    it('Carol exact output swap BUSD -> 1 BNB on stable 2pool', async () => {
      ...
      const exactOutput2PoolData = smartRouter.contract.methods
        .exactOutputStableSwap([BUSD.address, WBNB.address], [2], parseEther('1'), parseEther('10'))
        .encodeABI()
      const unwrapWETH9Data = smartRouter.contract.methods.unwrapWETH9(parseEther('1')).encodeABI()
      const result = await smartRouter.multicall(deadline, [exactOutput2PoolData, unwrapWETH9Data], {
        from: carol,
        gasPrice: 339817,
      })
    ...
    it('Carol exact output swap BNB -> 100 BUSD on stable 2pool', async () => {
      ...
      const exactOutputData = smartRouter.contract.methods
        .exactOutputStableSwap([WBNB.address, BUSD.address], [2], parseEther('100'), parseEther('100'))
        .encodeABI()
      const result = await smartRouter.multicall(deadline, [exactOutputData], {
        from: carol,
        value: inputBNBAmount,
        gasPrice: 260991,
      })
      ...
      assert.equal(
        (await tracker.delta()).abs().toString(),
        BigNumber.from(inputBNBAmount.toString()).add(gasCost).toString()
      )
